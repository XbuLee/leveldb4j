...
/**
 * <p>
 * A Snappy abstraction which attempts uses the iq80 implementation and falls back to the xerial
 * Snappy implementation it cannot be loaded.  You can change the load order by setting the
 * 'leveldb.snappy' system property.  Example:
 * <p/>
 * <code>
 * -Dleveldb.snappy=xerial,iq80
 * </code>
 * <p/>
 * The system property can also be configured with the name of a class which implements the
 * Snappy.SPI interface.
 * </p>
 *
...
public final class Snappy {
  ...
  private static final SnappySPI SNAPPY;

  static {
    SnappySPI attempt = null;
    ...
    for (int i = 0; i < factories.length && attempt == null; i++) {
      ...
      try {
        ...
        if ("xerial".equalsIgnoreCase(name)) {
          name = "cn.leexiaobu.leveledb.util.Snappy$XerialSnappy";
        } else if ("iq80".equalsIgnoreCase(name)) {
          name = "cn.leexiaobu.leveledb.util.Snappy$XerialSnappy";
//          name = "cn.leexiaobu.leveledb.util.Snappy$IQ80Snappy";
        }
        attempt = (SnappySPI) Thread.currentThread().getContextClassLoader().loadClass(name)
            .newInstance();
      } catch (Throwable e) {
      }
    }
  ...
  public static void uncompress(byte[] input, int inputOffset, int length, byte[] output,
      int outputOffset) throws IOException {
    SNAPPY.uncompress(input, inputOffset, length, output, outputOffset);
  }

  public static int compress(byte[] input, int inputOffset, int length, byte[] output,
      int outputOffset) throws IOException {
...
//  public static class IQ80Snappy implements SnappySPI {
//
//    static {
...
//      }
//
//      int count = org.iq80.snappy.Snappy.uncompress(input, inputOffset, length, output,
//          outputOffset);
//      if (uncompressed.hasArray()) {
...
//
//    @Override
//    public int uncompress(byte[] input, int inputOffset, int length, byte[] output,
//        int outputOffset) throws IOException {
...
//  }
  ...
  public static class XerialSnappy implements SnappySPI {
