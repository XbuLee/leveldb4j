...
public abstract class SliceOutput extends OutputStream implements DataOutput {
  ...
  /**
   * Returns {@code true} if and only if {@code (this.capacity - this.writerIndex)} is greater than
   * {@code 0}.
   */
   ...
   * {@code writerIndex} by {@code 1} in this buffer. The 24 high-order bits of the specified value
   * are ignored.
   *
  ...
  /**
   * Sets the specified 32-bit integer at the current {@code writerIndex} and increases the
   ...
   * {@code writerIndex} until the source buffer becomes unreadable, and increases the
   * {@code writerIndex} by the number of the transferred bytes.  This method is basically same with
   * {@link #writeBytes(Slice, int, int)}, except that this method increases the {@code readerIndex}
   * of the source buffer by the number of the transferred bytes while
  ...
  /**
   * Transfers the specified source buffer's data to this buffer starting at the current
   * {@code writerIndex} and increases the {@code writerIndex} by the number of the transferred
   * bytes (= {@code length}).  This method is basically same with
   * {@link #writeBytes(Slice, int, int)}, except that this method increases the {@code readerIndex}
   * of the source buffer by the number of the transferred bytes (= {@code length}) while
   * {@link #writeBytes(Slice, int, int)} does not.
   *
   * @param length the number of bytes to transfer
   * @throws IndexOutOfBoundsException if {@code length} is greater than {@code this.writableBytes}
   * or if {@code length} is greater then {@code source.readableBytes}
   */
   ...
   * if {@code sourceIndex + length} is greater than {@code source.capacity}, or if {@code length}
   ...
   * {@code writerIndex} and increases the {@code writerIndex} by the number of the transferred
   * bytes (= {@code source.length}).
   *
   * @throws IndexOutOfBoundsException if {@code source.length} is greater than
   * {@code this.writableBytes}
   */
   ...
   * @param length the number of bytes to transfer
   * @throws IndexOutOfBoundsException if the specified {@code sourceIndex} is less than {@code 0},
   * if {@code sourceIndex + length} is greater than {@code source.length}, or if {@code length} is
  ...
  /**
   * Transfers the specified source buffer's data to this buffer starting at the current
   * {@code writerIndex} until the source buffer's position reaches its limit, and increases the
   ...
   * {@code buf.toString(buf.readerIndex(), buf.readableBytes(), charsetName)}. This method does not
