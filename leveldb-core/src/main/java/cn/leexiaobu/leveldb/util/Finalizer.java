...
public class Finalizer<T> {
  ...
  public synchronized void addCleanup(T item, Callable<?> cleanup) {
    ...
    if (executor == null) {
      ...
      ThreadFactory threadFactory = new ThreadFactoryBuilder().setNameFormat(
          "FinalizerQueueProcessor-%d").setDaemon(true).build();
    ...
    FinalizerPhantomReference<T> reference = new FinalizerPhantomReference<>(item, referenceQueue,
        cleanup);
  ...
  private static class FinalizerPhantomReference<T> extends PhantomReference<T> {
    ...
    private FinalizerPhantomReference(T referent, ReferenceQueue<? super T> queue,
        Callable<?> cleanup) {
