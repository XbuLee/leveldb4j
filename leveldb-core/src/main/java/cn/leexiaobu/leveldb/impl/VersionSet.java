...
import static cn.leexiaobu.leveldb.impl.LogMonitors.throwExceptionMonitor;
...
import cn.leexiaobu.leveldb.util.InternalIterator;
import cn.leexiaobu.leveldb.util.Level0Iterator;
...
public class VersionSet implements SeekingIterable<InternalKey, Slice> {
  ...
  public void recover() throws IOException {
    ...
    try (FileInputStream fis = new FileInputStream(
        new File(databaseDir, currentName)); FileChannel fileChannel = fis.getChannel()) {
      // read log edit log
    ...
    if (sizeCompaction) {
      ...
      for (FileMetaData fileMetaData : current.getFiles(level)) {
        if (!compactPointers.containsKey(level)
            || internalKeyComparator.compare(fileMetaData.getLargest(), compactPointers.get(level))
            > 0) {
...
//        }
  ...
  public long getMaxNextLevelOverlappingBytes() {
    ...
    for (int level = 1; level < NUM_LEVELS; level++) {
      for (FileMetaData fileMetaData : current.getFiles(level)) {
        List<FileMetaData> overlaps = getOverlappingInputs(level + 1, fileMetaData.getSmallest(),
  ...
  private static class Builder {
    ...
    public void saveTo(Version version) throws IOException {
      FileMetaDataBySmallestKey cmp = new FileMetaDataBySmallestKey(
          versionSet.internalKeyComparator);
      ...
      if (levels.get(level).deletedFiles.contains(fileMetaData.getNumber())) {
        ...
        if (level > 0 && !files.isEmpty()) {
          ...
          boolean filesOverlap =
              versionSet.internalKeyComparator.compare(files.get(files.size() - 1).getLargest(),
    ...
    private static class LevelState {
      ...
      public LevelState(InternalKeyComparator internalKeyComparator) {
        addedFiles = new TreeSet<FileMetaData>(
            new FileMetaDataBySmallestKey(internalKeyComparator));
      }

      @Override
      public String toString() {
        String sb = "LevelState"
