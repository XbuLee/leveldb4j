...
public class Compaction {
  ...
  public Compaction(Version inputVersion, int level, List<FileMetaData> levelInputs,
      List<FileMetaData> levelUpInputs, List<FileMetaData> grandparents) {
  ...
  public boolean isTrivialMove() {
    ...
    return (levelInputs.size() == 1 && levelUpInputs.isEmpty()
  ...
  public boolean shouldStopBefore(InternalKey internalKey) {
    ...
    while (grandparentIndex < grandparents.size() &&
        internalKeyComparator.compare(internalKey, grandparents.get(grandparentIndex).getLargest())
            > 0) {
      if (seenKey) {
