...
import cn.leexiaobu.leveledb.util.InternalTableIterator;
import cn.leexiaobu.leveledb.util.LevelIterator;
...
public class Level implements SeekingIterable<InternalKey, Slice> {
  ...
  public static LevelIterator createLevelConcatIterator(TableCache tableCache,
      List<FileMetaData> files, InternalKeyComparator internalKeyComparator) {
  ...
  public LookupResult get(LookupKey key, ReadStats readStats) {
    ...
    if (levelNumber == 0) {
      ...
      int index = ceilingEntryIndex(Lists.transform(files, FileMetaData::getLargest),
          key.getInternalKey(), internalKeyComparator);
