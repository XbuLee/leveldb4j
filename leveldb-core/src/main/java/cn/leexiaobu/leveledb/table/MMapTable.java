...
import cn.leexiaobu.leveledb.util.ByteBufferSupport;
import cn.leexiaobu.leveledb.util.Closeables;
...
public class MMapTable extends Table {
  ...
  public MMapTable(String name, FileChannel fileChannel, Comparator<Slice> comparator,
      boolean verifyChecksums) throws IOException {
    super(name, fileChannel, comparator, verifyChecksums);
    checkArgument(fileChannel.size() <= Integer.MAX_VALUE, "File must be smaller than %s bytes",
        Integer.MAX_VALUE);
  }
  ...
  public static ByteBuffer read(MappedByteBuffer data, int offset, int length) throws IOException {
    ...
    ByteBuffer block = data.duplicate().order(ByteOrder.LITTLE_ENDIAN).clear()
  ...
  protected Footer init() throws IOException {
    ...
    Slice footerSlice = Slices.copiedBuffer(data, (int) size - Footer.ENCODED_LENGTH,
  ...
  protected Block readBlock(BlockHandle blockHandle) throws IOException {
    ...
    BlockTrailer blockTrailer = BlockTrailer.readBlockTrailer(
        Slices.copiedBuffer(this.data, (int) blockHandle.getOffset() + blockHandle.getDataSize(),
...
//        }
    ...
    ByteBuffer uncompressedBuffer = read(this.data, (int) blockHandle.getOffset(),
