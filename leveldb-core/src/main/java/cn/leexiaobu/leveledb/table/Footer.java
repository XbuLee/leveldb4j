...
import static cn.leexiaobu.leveledb.table.BlockHandle.readBlockHandle;
import static cn.leexiaobu.leveledb.table.BlockHandle.writeBlockHandleTo;
...
public class Footer {
  ...
  public static Footer readFooter(Slice slice) {
    ...
    checkArgument(slice.length() == ENCODED_LENGTH, "Expected slice.size to be %s but was %s",
        ENCODED_LENGTH, slice.length());
  ...
  public static void writeFooter(Footer footer, SliceOutput sliceOutput) {
    ...
    sliceOutput.writeZero(
        ENCODED_LENGTH - SIZE_OF_LONG - (sliceOutput.size() - startingWriteIndex));
