 ...
 */
package cn.leexiaobu.leveledb.table;

import static cn.leexiaobu.leveledb.impl.VersionSet.TARGET_FILE_SIZE;
import static com.google.common.base.Preconditions.checkState;
import static java.util.Objects.requireNonNull;

import cn.leexiaobu.leveldb.CompressionType;
import cn.leexiaobu.leveldb.Options;
import cn.leexiaobu.leveledb.util.PureJavaCrc32C;
...
public class TableBuilder {
  ...
  public TableBuilder(Options options, FileChannel fileChannel, UserComparator userComparator) {
    ...
    try {
      checkState(position == fileChannel.position(),
          "Expected position %s to equal fileChannel.position %s", position,
  ...
  public void add(Slice key, Slice value)
    ...
    checkState(!closed, "table is finished");
      ...
      assert entryCount <= 0 || (userComparator.compare(key, lastKey) > 0) : "key must be greater than last key";
  ...
  private BlockHandle writeBlock(BlockBuilder blockBuilder)
    ...
    if (compressionType == CompressionType.SNAPPY) {
      ...
      try {
        int compressedSize = Snappy.compress(raw.getRawArray(), raw.getRawOffset(), raw.length(),
    ...
    BlockTrailer blockTrailer = new BlockTrailer(blockCompressionType,
    ...
    position += fileChannel.write(
        new ByteBuffer[]{blockContents.toByteBuffer(), trailer.toByteBuffer()});
  ...
  public void finish()
    ...
    BlockBuilder metaIndexBlockBuilder = new BlockBuilder(256, blockRestartInterval,
